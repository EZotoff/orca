...
import { applyProbeFilter, isProbeCard } from './probe-card-filter'
...
function snapshot(cards: DashboardCard[]): DashboardSnapshot {
  return { generatedAt: 0, cards }
}

describe('overview panel probe filter (contract clause f)', () => {
  it('drops PROBE-OK-class cards regardless of prefix', () => {
    expect(isProbeCard(card({ paneKey: 'a', conversationName: 'PROBE-OK' }))).toBe(true)
    expect(isProbeCard(card({ paneKey: 'b', conversationName: 'GLM-SJ-PROBE-OK probe' }))).toBe(
      true
    )
    expect(isProbeCard(card({ paneKey: 'c', conversationName: 'K3-PROBE-OK reply test' }))).toBe(
      true
    )
  ...
  it('keeps real attention cards', () => {
    expect(isProbeCard(card({ paneKey: 'a', conversationName: 'Fix login regression' }))).toBe(
      false
    )
