...
agent is doing, unless the main agent is itself asking. Two known divergences
remain, pinned by name in the parity table
...
- The Claude hook lane records a child's permission wait by displacing the
  main agent record (`waitingAgentId`, `stateBeforeWait`), so its
  `mainAgent.state` reads `waiting` while the main agent is really still
  working or done. The Codex lane keeps the wait on the child, and its
  `mainAgent` stays the root's own state. Moving Claude onto the child fact
  flips its rows in that table.
